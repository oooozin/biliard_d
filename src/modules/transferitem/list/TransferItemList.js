...
import { setPaginate } from "../transferItemSlice";
...
export const TransferItemList = () => {
    const { transferItems, paginateParams } = useSelector((state) => state.transferItem);
    ...
    const [columns, setColumns] = useState(getData(transferItemPayload.columnsName) == null ? transferItemPayload.columns : getData(transferItemPayload.columnsName));

    const transferItemStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(transferItemPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await transferItemService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await transferItemService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await transferItemService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await transferItemService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await transferItemService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await transferItemService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await transferItemService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(transferItemPayload.columnsName) == null){
            setData(transferItemPayload.columnsName, transferItemPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(transferItemPayload.columnsName, columns)
    },[columns])
                                        ...
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTransferItems="center">
                                                ...
                                                <Grid transferItem xs={1}>
                                                    <TableCustomizeSetting payload={transferItemPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.transferItem}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
