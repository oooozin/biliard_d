...
import { setPaginate } from "../materialDataSlice";
...
export const MaterialDataList = () => {
    const { materialDatas, paginateParams } = useSelector((state) => state.materialData);
    ...
    const [columns, setColumns] = useState(getData(materialDataPayload.columnsName) == null ? materialDataPayload.columns : getData(materialDataPayload.columnsName));

    const materialDataStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(materialDataPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await materialDataService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await materialDataService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await materialDataService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await materialDataService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await materialDataService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await materialDataService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await materialDataService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(materialDataPayload.columnsName) == null){
            setData(materialDataPayload.columnsName, materialDataPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(materialDataPayload.columnsName, columns)
    },[columns])
                                ...
                                <TableRow>
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignMaterialDatas="center">
                                                
                                                <Grid materialData xs={1}>
                                                    <TableCustomizeSetting payload={materialDataPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                {/* <Grid materialData xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={materialDataStatus} onFilter={onFilter} />
                                                </Grid> */}

                                                <Grid materialData xs={8}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={5} xl={5} direction="row" justifyContent="flex-end" alignMaterialDatas="center">
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.materialData}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
