...
export const ItemList = () => {
    ...
    const [columns, setColumns] = useState(getData(itemPayload.columnsName) == null ? itemPayload.columns : getData(itemPayload.columnsName));

    const itemStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await itemService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await itemService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await itemService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await itemService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await itemService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await itemService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        ...
        setIsLoading(false)
        if(getData(itemPayload.columnsName) == null){
            setData(itemPayload.columnsName, itemPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(itemPayload.columnsName, columns)
    },[columns])
                                    ...
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignItems="center">
                                                ...
                                                <Grid item xs={1}>
                                                    <TableCustomizeSetting payload={itemPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid item xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={itemStatus} onFilter={onFilter} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.item}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
