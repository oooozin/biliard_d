...
export const CategoryList = () => {
    ...
    const [columns, setColumns] = useState(getData(categoryPayload.columnsName) == null ? categoryPayload.columns : getData(categoryPayload.columnsName));

    const categoryStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(categoryPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await categoryService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await categoryService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await categoryService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await categoryService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await categoryService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await categoryService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        ...
        setIsLoading(false)
        if(getData(categoryPayload.columnsName) == null){
            setData(categoryPayload.columnsName, categoryPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(categoryPayload.columnsName, columns)
    },[columns])
                                    ...
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignItems="center">
                                                ...
                                                <Grid item xs={1}>
                                                    <TableCustomizeSetting payload={categoryPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid item xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={categoryStatus} onFilter={onFilter} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.category}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
