...
export const OrderList = () => {
    ...
    const [columns, setColumns] = useState(getData(orderPayload.columnsName) == null ? orderPayload.columns : getData(orderPayload.columnsName));

    const orderStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    const orderShopStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await orderService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await orderService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await orderService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await orderService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await orderService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await orderService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        ...
        setIsLoading(false)
        if(getData(orderPayload.columnsName) == null){
            setData(orderPayload.columnsName, orderPayload.columns)
        }
        const shopResult = await getRequest(`${endpoints.shop}`);
        if (shopResult.status === 200) {
            orderShopStatus.current = shopResult.data.filter(item => item.is_warehouse !== true);
            orderShopStatus.current.unshift({ id: '', name: 'ALL' })
        }
    ...
    useEffect(()=>{
        setData(orderPayload.columnsName, columns)
    },[columns])
                                    ...
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTables="center">
                                                ...
                                                <Grid order xs={1}>
                                                    <TableCustomizeSetting payload={orderPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid order xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={orderStatus} onFilter={onFilter} />
                                                </Grid>

                                                <Grid itemData xs={2}> 
                                                    <FilterByForign paginateParams={paginateParams} status={orderShopStatus} onFilter={onFilterShop} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.order}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
