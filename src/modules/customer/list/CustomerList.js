...
export const CustomerList = () => {
    ...
    const [columns, setColumns] = useState(getData(customerPayload.columnsName) == null ? customerPayload.columns : getData(customerPayload.columnsName));

    const customerStatus = useRef(['ALL']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(customerPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await customerService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await customerService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await customerService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await customerService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await customerService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await customerService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await customerService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(customerPayload.columnsName) == null){
            setData(customerPayload.columnsName, customerPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(customerPayload.columnsName, columns)
    },[columns])
                                        ...
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTransferItems="center">
                                                ...
                                                <Grid transferItem xs={1}>
                                                    <TableCustomizeSetting payload={customerPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.customer}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
