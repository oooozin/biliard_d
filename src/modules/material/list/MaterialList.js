...
import { setPaginate } from "../materialSlice";
...
export const MaterialList = () => {
    const { materials, paginateParams } = useSelector((state) => state.material);
    ...
    const [columns, setColumns] = useState(getData(materialPayload.columnsName) == null ? materialPayload.columns : getData(materialPayload.columnsName));

    const materialStatus = useRef(['ALL','ACTIVE','INACTIVE']);
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(materialPayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await materialService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await materialService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await materialService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await materialService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await materialService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await materialService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await materialService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(materialPayload.columnsName) == null){
            setData(materialPayload.columnsName, materialPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(materialPayload.columnsName, columns)
    },[columns])
                                ...
                                <TableRow>
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignMaterials="center">
                                                
                                                <Grid material xs={1}>
                                                    <TableCustomizeSetting payload={materialPayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>

                                                <Grid material xs={2}> 
                                                    <FilterByStatus paginateParams={paginateParams} status={materialStatus} onFilter={onFilter} />
                                                </Grid>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={5} xl={5} direction="row" justifyContent="flex-end" alignMaterials="center">
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.material}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
