...
export const PackageList = () => {
    ...
    const [columns, setColumns] = useState(getData(packagePayload.columnsName) == null ? packagePayload.columns : getData(packagePayload.columnsName));

    const packageStatus = useRef(['ALL']);
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await packageService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await packageService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await packageService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await packageService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await packageService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await packageService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await packageService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(packagePayload.columnsName) == null){
            setData(packagePayload.columnsName, packagePayload.columns)
        }
    ...
    useEffect(()=>{
        setData(packagePayload.columnsName, columns)
    },[columns])
                                        ...
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTransferItems="center">
                                                ...
                                                <Grid transferItem xs={1}>
                                                    <TableCustomizeSetting payload={packagePayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.package}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
